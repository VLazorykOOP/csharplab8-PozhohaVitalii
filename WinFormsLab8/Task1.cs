...
namespace WinFormsLab8
{
    public partial class Task1 : Form
    {
        ...
        private void ProcessTextFile(string inputFile, string outputFile, string replaceString)
        {
            // Читаємо вміст вхідного файлу
            string inputText = File.ReadAllText(inputFile);

            // Знаходимо всі адреси web-сайтів домена com за допомогою регулярних виразів
            string pattern = @"(\b\w+\.com\b)";
            MatchCollection matches = Regex.Matches(inputText, pattern);

            // Підраховуємо кількість знайдених адрес
            int count = matches.Count;

            // Замінюємо адреси за вказаним параметром
            string modifiedText = Regex.Replace(inputText, pattern, replaceString);

            // Записуємо змінений текст у новий файл
            File.WriteAllText(outputFile, modifiedText);

            MessageBox.Show($"Знайдено {count} адрес com та замінено за вказаним параметром.");
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (!selectedFile.Equals("") && !selectedFile2.Equals(""))
            {
                ...
                ProcessTextFile(inputFile, outputFile, replaceString);
            }
            else
            {
